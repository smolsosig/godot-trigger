...
The Trigger2D/3D nodes are essentially just extended Area2D/3D nodes, which should allow for easier integration with already existing projects.

Interacting with the triggers fires off a list of outputs specified in the Inspector. An output simply calls a method to a Node.
...
The way outputs are handled are made to be similar to how Source handles outputs.
...
* On that note, as the trigger relies on NodePaths, there are also currently no ways to reference an Autoload, though this can be circumvented by way of a proxy Node.
